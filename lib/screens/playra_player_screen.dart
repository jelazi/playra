...
class _PlayraPlayerScreenState extends State<PlayraPlayerScreen> {
  ...
  void _onHorizontalDrag(DragUpdateDetails d) {
    ...
    final clamped = Duration(
      milliseconds: target.inMilliseconds.clamp(0, _duration.inMilliseconds == 0 ? 1 : _duration.inMilliseconds),
    );
    _player.seek(clamped);
    _flashOverlay(
      delta.isNegative ? Icons.fast_rewind : Icons.fast_forward,
            ...
            children: [
                    ...
                    Expanded(
                      child: GestureDetector(
                        ...
                        onDoubleTap: () => _seekRelative(
                          Duration(seconds: -PlayraStorage.getPlayerSettings().seekStepSeconds.toInt()),
                        ),
                    ...
                    child: Row(
                      ...
                      children: [
                        if (_overlayIcon != null) ...[
                          Icon(_overlayIcon, color: Colors.white, size: 28),
              ...
              if (_showControls)
                Positioned(
                  ...
                  left: 0,
                  right: 0,
                  child: _buildBottomBar(mediaSize),
                ),
            ...
            ? [
                for (final dx in [-s.outlineWidth, 0.0, s.outlineWidth])
                  for (final dy in [-s.outlineWidth, 0.0, s.outlineWidth])
                    if (dx != 0 || dy != 0)
                      Shadow(offset: Offset(dx, dy), color: Color(s.outlineColor), blurRadius: 0),
              ]
            : null,
      ),
  ...
  Future<void> _pickSubtitleTrack() async {
    ...
    final picked = await _showTrackPicker<SubtitleTrack>(
      title: 'player.subtitle_track'.tr(),
      tracks: tracks,
      current: current,
      label: (t) => t.title ?? t.language ?? t.id,
    );
